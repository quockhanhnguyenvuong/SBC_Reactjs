...
function MapDoctor(props) {
  ...
  const renderPatientInfo = () => {
    return patientPositions.slice(1, 10000).map((patient, index) => {
      const distance = haversine(doctorPosition, patient) / 1000; // Quãng đường tính bằng mét, chuyển sang km
      const speed = 40; // Giả sử vận tốc là 40 km/h
      const time = (distance / speed) * 60; // Thời gian tính theo giờ
