...
class ModalForgotPassword extends Component {
  ...
  checkValidateInputCofirm = () => {
    ...
    let arrInput = ["email", "code", "newPassword", "checkNewPassword"];
    for (let i = 0; i < arrInput.length; i++) {
      if (!this.state[arrInput[i]]) {
        isValue = false;
        if (arrInput[i] === "email") toast.warn("Vui lòng nhập email của bạn");
        if (arrInput[i] === "code") toast.warn("Vui lòng nhập mã OTP của bạn");
        if (arrInput[i] === "newPassword")
          toast.warn("Vui lòng nhập mật khẩu mới của bạn");
        if (arrInput[i] === "checkNewPassword")
          toast.warn("Vui lòng nhập xác nhận mật khẩu mới của bạn");
        break;
      }
    }
    return isValue;
  ...
  checkOTP = async (data) => {
    try {
      ...
      if (
        response_Otp &&
        response_Otp.errCode == 0 &&
        response_Password &&
        response_Password.errCode == 0
      ) {
      ...
      >
        ...
        <ModalBody>
          <div className="modal-body w-100 d-flex justify-content-center modal-body">
            ...
            <div className="input-container w-100  ">
              <label className="code-confirm label-text">
                Nhập mã xác nhận:
              </label>
              <div className="col2">
                <input
            ...
            <div className="input-container w-100  ">
              <label className="new-password label-text">
                Nhập mật khẩu mới:
              </label>
              <div className="col2">
                <input
            ...
            <div className="input-container w-100  ">
              <label className="Confirm-password label-text">
